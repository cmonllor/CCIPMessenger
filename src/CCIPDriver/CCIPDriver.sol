...
import "./ICCIPDriverConsumer.sol";

using SafeERC20 for IERC20;

contract CCIPDriver is CCIPReceiver, OwnerIsCreator {
    ...
    // `Sent` indicates that a message has been sent to the destination CCIPDriver contract but not yet acknowledged.
    // `Received` indicates that the a message has been received.
    ...
    // `Failed` indicates that the message has failed to be sent.
    // `Acknowledged` indicates that the Acknowledgment message has been sent back to the sender.
    enum MessageStatus {
        NotSent, // 0
        Sent, // 1
        Received, // 2
        ProcessedOnDestination, // 3
        Failed, // 5
        Acknowledged // 6
    }

    // Enum to differerentiate between the types of messages sent.
    enum MessageType {
        Message, // 0
        Acknowledgment // 1
    }
    ...
    {
        bytes32 receivedMessageId = any2EvmMessage.messageId; // The message ID of the received message
        Message memory message = abi.decode(any2EvmMessage.data, (Message)); // Decode the data sent by the receiver
        MessageType messageType = message.messageType;
        address sender = abi.decode(any2EvmMessage.sender, (address));
        uint64 sourceChainSelector = any2EvmMessage.sourceChainSelector;

        if ( messageType == MessageType.Message ) {
            emit MessageReceived(receivedMessageId, sourceChainSelector, sender, message.data);
            messagesInfo[receivedMessageId].status = MessageStatus.Received;
            if (ICCIPDriverConsumer(owner()).processMessage(receivedMessageId, sender, sourceChainSelector, message.data)) {
                acknowledgePayLINK(receivedMessageId, sender, sourceChainSelector);
                messagesInfo[receivedMessageId].status = MessageStatus.Acknowledged;                
            }
            else {
                ICCIPDriverConsumer(owner()).processError(receivedMessageId, sender, sourceChainSelector, message.data);
                messagesInfo[receivedMessageId].status = MessageStatus.Failed;
            }

        } else if ( messageType == MessageType.Acknowledgment ) {
            ICCIPDriverConsumer(owner()).processAcknowledgment(receivedMessageId, sender, sourceChainSelector, message.data);
        }
    }

    function acknowledgePayLINK(
        ...
        Client.EVM2AnyMessage memory acknowledgment = Client.EVM2AnyMessage({
            ...
            data: abi.encode(mssg), // ABI-encoded {ACK_Code, message ID to acknowledge}
